...
(function () {
  ...
  function applyCssFallback(host) {
    if (!host || host.getAttribute("data-neoflex-css-bg") === "1") return;
    host.setAttribute("data-neoflex-css-bg", "1");
    host.classList.add("neoflex-hero-css-fallback");
  ...
  function mount() {
    var el = document.getElementById("neoflex-hero-shader");
    if (!el) return false;
    if (el.querySelector("canvas") || el.getAttribute("data-neoflex-css-bg") === "1") return true;
  ...
  function tryMount() {
    if (mounted && (document.getElementById("neoflex-hero-shader") || usingCssFallback)) return;
    if (mount()) {
      mounted = true;
      if (obs) {
        obs.disconnect();
